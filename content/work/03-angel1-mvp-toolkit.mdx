...
**No update path for existing projects.** If I improve the template, projects scaffolded last month don't see the improvement without manually diffing. A `claude-mvp-toolkit upgrade` command that applies non-conflicting changes is on the list of things real users would justify building.
...
The eval harness that earlier drafts of this case study promised? It's been extracted into a separate tool: **[angel1-rag-eval](/work/04-angel1-rag-eval)**. Two tools, one workflow: scaffold with the toolkit, measure with the eval CLI.
