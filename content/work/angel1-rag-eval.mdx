...
- **core/scorer** — retrieval precision @K against the labeled `expected_source` set, plus the orchestration glue in `core/runner.ts` and `core/judge.ts`.
...
- **formatters/{table,csv,json}** — three output formats. Table renders to stdout via `cli-table3`. CSV and JSON are written to disk when `--output <dir>` is set, producing `eval-<timestamp>.csv` and `eval-<timestamp>.json` side by side.
