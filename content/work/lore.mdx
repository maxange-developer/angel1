...
Three threads, in priority order. An agent layer that can act, not just answer — file a ticket on the user's behalf, escalate, draft a reply for an agent to ship. Continuous evaluation in CI via [angel1-rag-eval](/work/angel1-rag-eval) — the companion tool that came out of this build. A labelled question set scoped to this domain, scored on retrieval precision, faithfulness, and correctness on every PR. And an export pipeline that lets a tenant pull their corpus and embeddings if they ever want to leave. The last one is non-negotiable — if you can't leave a system you don't trust it.

---

Live at [lore.massimilianoangelone.com](https://lore.massimilianoangelone.com) · Code on [GitHub](https://github.com/maxange-developer/lore)
